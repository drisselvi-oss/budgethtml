...
function getMonthBalance(m) {
  return getTotalIncome(m) - getTotalExpenses(m) - getAnnualTotalForMonth(m) - getMonthSavings(m);
}
...
function renderMonthPage(m) {
  ...
  var balance = totalIncome - totalExpenses - annualTotal - savingsAmount;
  var totalOut = totalExpenses + annualTotal;
...
function renderAnnualPage() {
  ...
  var totalIncomeYear = 0, totalExpensesYear = 0;
